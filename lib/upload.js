...
module.exports = ({
  ...
  return async (req) => {
    ...
    try {
      ...
      const destinationDir = generateRelativePath(req);
      const fileLocation = path.resolve(uploadDir, destinationDir);
      const destFile = await file.move(fileLocation, fileDesc);
