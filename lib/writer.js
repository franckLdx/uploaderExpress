...
module.exports.reqToFile = function reqToFile(req, intFile) {
  return co(function* reqToFileCo() {
    let fd;
    try {
      fd = yield fsp.open(
        intFile.fullPath,
      ...
      try { yield fsp.close(fd); } catch (err2) { /* The first error matter, not this one */ }
      throw err;
    }
  });
};
